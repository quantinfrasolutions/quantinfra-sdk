...
public abstract class AbstractHostedStrategy
{
    ...
    protected AbstractHostedStrategy(StrategyConfig s)
    {
        StrategyConfig = s;
    ...
    )
    {
        ...
        if (positionAccounting == PositionAccounting.Netted && positionEffect != PositionEffect.Unknown)
        {
        ...
        {
            throw new ArgumentException("Position effect must be specified when using the Hedged accounting");
        }
    ...
    protected void ClosePosition(string symbol, decimal newVolume = 0, string? positionId = null, string? clOrdId = null, int? orderValiditySeconds = null, int[]? tradingSessionIds = null)
    {
        ...
        if (newVolume != 0)
        {
            throw new NotImplementedException("Partial closing a position is not yet supported");
    ...
    protected decimal GetVolume(int contractId, decimal value)
    {
        ...
        var contract = Context!.GetContract(contractId);
        return contract.PLCalculator.GetVolume(value, (decimal)lastPrice);
